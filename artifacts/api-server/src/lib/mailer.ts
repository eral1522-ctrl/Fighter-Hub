...
export async function sendGbsInterestNotification(details: {
  category: "attendee" | "partner" | "speaker";
  name: string;
  email: string;
  ...
  if (!getSmtpDiagnostics().configured) {
    await logEmail({ emailType: "gbs_interest", recipientEmail: details.email, success: false, errorMessage: "Skipped: SMTP not configured" });
    throw new SmtpDeliveryError("SMTP is not configured on this server", "Missing SMTP variable");
  }
  ...
  const categoryLabel =
    details.category === "attendee" ? "Register Interest (Attendee)" :
    details.category === "partner" ? "Partner Interest" :
    "Speaker Interest";
...
<body style="background:#0d0d0d;color:#f5f5f5;font-family:Arial,sans-serif;padding:40px 20px;margin:0;">
  <div style="max-width:560px;margin:0 auto;">
    ${header()}
    <div style="border-top:2px solid #c61010;padding-top:24px;margin-bottom:24px;">
      <h2 style="font-size:22px;font-weight:900;text-transform:uppercase;letter-spacing:2px;margin:0 0 16px;">New GBS Interest Submission</h2>
      <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
        ${fieldRow("Category", esc(categoryLabel))}
        ${fieldRow("Name", esc(details.name))}
        ${fieldRow("Email", `<a href="mailto:${esc(details.email)}" style="color:#c61010;text-decoration:none;">${esc(details.email)}</a>`)}
        ${details.organization ? fieldRow("Organization", esc(details.organization)) : ""}
      </table>
      ${details.message ? `
      <div style="background:#111;border:1px solid #333;border-radius:4px;padding:16px;">
        <p style="color:#888;font-size:11px;text-transform:uppercase;letter-spacing:2px;margin:0 0 10px;">Message</p>
      ...
      <p style="color:#666;font-size:12px;margin-top:20px;">
        Submitted via the Global Boxing Summit page. Reply directly to this email to respond to ${esc(details.name)}.
      </p>
    </div>
    ${footer()}
  </div>
  ...
  try {
    await deliver({
      ...
      to: recipient,
      replyTo: details.email,
      subject: `[GBS] ${categoryLabel} — ${details.name}`,
      html,
    });
    await logEmail({ emailType: "gbs_interest", recipientEmail: recipient, success: true });
  } catch (err) {
    await logEmail({
      emailType: "gbs_interest",
      recipientEmail: recipient,
      success: false,
      errorMessage: sanitizeError(err),
    });
